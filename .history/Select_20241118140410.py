...
class GraphicsEditor:
    def __init__(self, root):
        ...
        self.offset_x = 0
        self.offset_y = 0
        self.image = None  # Placeholder for imported image
    ...
    def init_ui(self):
        ...
        tools = [
            ...
            ("Select", self.select_item_tool),
            ("Deselect", self.deselect_item_tool),
            ("Copy", self.copy_item)
        ]
        ...
        self.zoom_slider = tk.Scale(toolbar, from_=20, to=500, orient=tk.HORIZONTAL, showvalue=1, command=self.update_zoom)
        self.zoom_slider.set(100)  # Default zoom level: 100%
        self.zoom_slider.pack(side=tk.LEFT, padx=2, pady=2)
    ...
    def select_item_tool(self):
        self.select_tool("select")

    def deselect_item_tool(self):
        if self.selected_item:
            self.canvas.itemconfig(self.selected_item, outline=self.current_color)
            self.selected_item = None
    ...
    def copy_item(self):
        if self.selected_item:
            ...
            self.selected_item = None
            new_item = self.canvas.create_rectangle(*coords, outline=self.current_color)
    ...
    def on_click(self, event):
        ...
        if self.current_tool == "select":
            ...
            if item:
                ...
                self.offset_x = event.x - self.canvas.coords(self.selected_item)[0]
                self.offset_y = event.y - self.canvas.coords(self.selected_item)[1]
                self.canvas.itemconfig(self.selected_item, outline="red")  # Highlight selected item
    ...
    def zoom_in(self):
        if self.zoom_level < 5.0:  # Limit zoom in
            self.zoom_level *= 1.2
            self.update_canvas_scale()

    def zoom_out(self):
        if self.zoom_level > 0.2:  # Limit zoom out
            self.zoom_level /= 1.2
            self.update_canvas_scale()

    def update_zoom(self, value):
        new_zoom = float(value) / 100
    ...
    def update_canvas_scale(self):
        ...
        self.zoom_slider.set(self.zoom_level * 100)
