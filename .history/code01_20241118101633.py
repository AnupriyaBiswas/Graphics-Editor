    ...
    def on_drag(self, event):
        if self.current_tool:
            if self.current_tool == "line" and self.start_x and self.start_y:
                ...
                self.current_item = self.canvas.create_line(self.start_x, self.start_y, event.x, event.y, fill=self.current_color)
            elif self.current_tool == "rectangle" and self.start_x and self.start_y:
                self.canvas.delete(self.current_item)
                self.current_item = self.canvas.create_rectangle(self.start_x, self.start_y, event.x, event.y, outline=self.current_color)
            elif self.current_tool == "ellipse" and self.start_x and self.start_y:
                self.canvas.delete(self.current_item)
                self.current_item = self.canvas.create_oval(self.start_x, self.start_y, event.x, event.y, outline=self.current_color)
            elif self.current_tool == "pencil":
                self.pencil_coords += [event.x, event.y]
